 <div class="row"> <!-- pocetak info i promo boxeva -->
      <div class="col-md-6"><!-- pocetak levog boxa -->
        <div class="jumbotron">
          <h2>Citati</h2>
          <blockquote>
          <p id="citat">Ovde napisati najvažnije i najbitnije informacije o kompaniji koje sajt kao celina sadrži...</p>
          <small><cite id="author"></cite></small>
          </blockquote>
          
        </div>
      </div><!-- kraj levog boxa -->
      <div class="col-md-6"><!-- pocetak desnog boxa -->
       <div class="jumbotron">
         <h2>Promocije</h2>
         <p>Ovde napisati najvažnije i najbitnije promocije o kompaniji koje sajt kao celina sadrži...</p>
         <p><a class="btn btn-danger btn-lg" href="#" role="button">Opširnije &#8649;</a></p>
       </div>
     </div><!-- kraj desnog boxa -->
   </div><!-- kraj info i promo boxeva -->